...
ApplicationWindow {
    ...
    // ── Background: velvet, burgundy, candlelight ──
    Rectangle {
        anchors.fill: parent
        gradient: Gradient {
    ...
    // ── Ambient floating particles (motes in candlelight) ──
    Repeater {
        model: 30
        Rectangle {
            id: dustParticle
            ...
            NumberAnimation on y {
                loops: Animation.Infinite
                from: dustParticle.startY
                to: dustParticle.startY - dustParticle.drift
                duration: dustParticle.dur
                easing.type: Easing.InOutSine
            }
            NumberAnimation on x {
                loops: Animation.Infinite
                from: dustParticle.startX - 10
                to: dustParticle.startX + 10
                duration: dustParticle.dur * 0.7
                easing.type: Easing.InOutSine
            }
        }
    }
    ...
    Component {
        ...
        GameView {
            ...
            onBackToTitle: {
                viewStack.pop(null) // pop to root
            }
        }
    }
